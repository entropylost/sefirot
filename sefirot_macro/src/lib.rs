...
impl VisitMut for TraceVisitor {
    ...
    fn visit_expr_mut(&mut self, node: &mut Expr) {
        ...
        let element_path = &self.element_path;
        let span = node.span();
                ...
                if let Expr::Let(_) = **cond {
                    ...
                    *node = parse_quote_spanned! {span=>
                        #debug_path::with_lineno("if",
                            ...
                            line!(),
                            column!(),
                            || <_ as #trait_path::SelectMaybeExpr<_>>::if_then_else(#cond,
                                #element_path::__block(|| #then_branch),
                                #element_path::__block(|| #else_branch)))
                    }
                } else {
                    *node = parse_quote_spanned! {span=>
                        #debug_path::with_lineno("if",
                            ...
                            line!(),
                            column!(),
                            || <_ as #trait_path::ActivateMaybeExpr>::activate(#cond,
                                #element_path::__block(|| #then_branch)))
                    }
                }
            }
            Expr::While(expr) => {
                ...
                *node = parse_quote_spanned! {span=>
                    #debug_path::with_lineno("while",
                            file!(),
                            line!(),
                            column!(), ||<_ as #trait_path::LoopMaybeExpr>::while_loop(
                                #element_path::__block(|| #cond),
                                #element_path::__block(|| #body)))
                }
            }
            Expr::Loop(expr) => {
                let body = &expr.body;
                *node = parse_quote_spanned! {span=>
                    #debug_path::with_lineno("loop",
                        file!(),
                        line!(),
                        column!(), || #flow_path::loop_(#element_path::__block(|| #body)))
                }
            }
            Expr::ForLoop(expr) => {
                let pat = &expr.pat;
    ...
    let body = quote_spanned! {body_span=>
        {
            ...
            #crate_path::luisa::lang::debug::comment(&format!("begin fn {} at {}:{}:{}", __fn_name, file!(), line!(), column!()));
            #[allow(clippy::let_unit_value)]
            let __ret: #ret_type = #body;
            #[allow(unreachable_code)]
            {
                #crate_path::luisa::lang::debug::comment(&format!("end fn {} at {}:{}:{}", __fn_name, file!(), line!(), column!()));
                __ret
            }
        }
    };
...
fn track_impl(mut ast: Expr, crate_path: &TokenStream) -> TokenStream {
    (TraceVisitor {
        is_last_stmt: false,
        flow_path: quote!(#crate_path::luisa::lang::control_flow),
        trait_path: quote!(#crate_path::luisa::lang::ops),
        debug_path: quote!(#crate_path::luisa::lang::debug),
        element_path: quote!(#crate_path::element),
    })
