...
pub trait DynAccessor<T: EmanationType> {
    ...
    fn save(&self, element: &mut Element<T>, field: RawFieldHandle);
...
pub trait Accessor<T: EmanationType>: 'static {
    type V: Any;
    type C: Any;
    fn get(&self, element: &mut Element<T>, field: Field<Self::V, T>)
    ...
    fn insert_cache(&self, element: &mut Element<T>, field: Field<Self::V, T>, value: Self::C) {
        element.cache.insert(field.raw, Box::new(value));
    }
    fn get_cache<'a>(
        &'a self,
        ...
        element
            .cache
            .get_mut(&field.raw)
            .and_then(move |x| x.downcast_mut())
    }
